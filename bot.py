...
def load_cipher():
    ...
    try:
        ...
        # fail closed: silently downgrading confessions to plaintext is not
        # an acceptable default (audit finding). Explicit opt-out only.
        if SECRETS.get('PLAINTEXT_OK') == '1':
            print('cryptography lib missing: PLAINTEXT_OK=1 set, logbooks '
                  'stored in PLAINTEXT by explicit operator choice', flush=True)
            return None
        sys.exit('cryptography lib missing: install it, or set PLAINTEXT_OK=1 '
                 'in secrets.env to accept plaintext logbooks explicitly')
    ...
    try:
        os.chmod(BASE / 'secrets.env', 0o600)
    except OSError:
        pass
...
WELCOME = (
    ...
    "for real. Replies are generated by an external model provider (/about "
    "names it, with the honest privacy status). We run no ads, never sell "
    "your data, never train on your content.\n"
...
def about_text():
    return (
        "Model: %s, called on the API at %s.\n"
        ...
        "Your data: /export and /delete, both real."
        # derived from the live configuration, so this can never silently
        # contradict where messages actually go (audit finding)
        % (MODEL, urllib.parse.urlparse(LLM_URL).hostname or LLM_URL)
    )
...
def upath(uid):
    """Read path: never creates anything. Reads must not create data
    (a read after /delete would silently resurrect the user directory)."""
    return DATA / str(uid)


def udir(uid):
    """Write path: creates the directory. Writers only."""
    d = upath(uid)
...
def user_reports(uid):
    """The user's own /report lines: part of the export inventory."""
    f = DATA / '_reports.jsonl'
    out = []
    if f.exists():
        for line in f.read_text().splitlines():
            try:
...
def purge_user_reports(uid):
    """Part of /delete: 'everything' includes the global reports file."""
    f = DATA / '_reports.jsonl'
    if not f.exists():
        return
    kept = []
    for line in f.read_text().splitlines():
        try:
            if str(json.loads(line).get('uid')) == str(uid):
                continue
        except ValueError:
            pass
        kept.append(line)
...
def think(uid, user_text, hist=None):
    """hist: history captured BEFORE the current text was logged. If omitted
    it is read now, which would duplicate the current message when the caller
    already logged it (audit finding: the model received every user message
    twice)."""
...
def handle_command(uid, chat, text):
    ...
    if cmd == '/start':
        ...
        # everything = the whole user-data inventory: logbook, profile, and
        # the user's own /report lines (audit finding: reports and profile
        # were missing, so "everything" was false)
        rows = []
        f = upath(uid) / 'logbook.jsonl'
        if f.exists():
            for line in open(f):
        ...
        p = profile(uid)
        if p:
            rows.append({'kind': 'profile', 'profile': p})
        for r in user_reports(uid):
            rows.append({'kind': 'report', **r})
        if rows:
            ...
            try:
                ...
                # the decrypted file never survives, even when sending fails
                if out.exists():
                    out.unlink()
        else:
        ...
        # only claim what actually happened (audit finding)
        if forwarded:
            send(chat, S(uid, 'REPORT_ACK',
                         'Logged and forwarded to the operator. Thank you - '
                         'reports like this are how the beta gets better.'))
        else:
            send(chat, S(uid, 'REPORT_ACK_LOCAL',
                         'Logged. Thank you - reports like this are how the '
                         'beta gets better.'))
    elif cmd == '/anchors':
        p = profile(uid)
...
def handle_message(uid, chat, text):
    ...
    if p.get('pending_delete'):
        if text.strip() == 'DELETE':
            ...
            shutil.rmtree(upath(uid), ignore_errors=True)
            purge_user_reports(uid)
            send(chat, goodbye)
            return
    ...
    hist = recent_messages(uid)  # before logging, so the model gets the current text once
    log(uid, 'user', text)
    try:
        tg('sendChatAction', chat_id=chat, action='typing', timeout=10)
    except Exception:
        pass
    reply = think(uid, text, hist)
    if reply is None:
        send(chat, FALLBACK)
...
def anchors_loop():
    ...
    while True:
        ...
        if now.hour == ANCHOR_HOUR and DATA.exists():
            # only numeric user directories: DATA also holds metadata files
            # (_offset, _reports.jsonl) which killed this thread permanently
            # when treated as users (audit finding)
            for d in DATA.iterdir():
                uid = d.name
                if not (d.is_dir() and uid.isdigit()):
                    continue
...
def main():
    ...
    while True:
        ...
        for u in updates.get('result', []):
            ...
            # private chats only: in a group, replies and /export would
            # publish someone's logbook to the room (audit finding)
        ...
        if updates.get('result'):
            # persist the offset only after the batch is dispatched: writing
            # it first meant a crash silently dropped every queued message
            # (audit finding). Redelivery after a crash beats silence.
...
def selftest():
    """Dry-run of the state machine with send() stubbed. No network, and no
    production data: DATA is repointed to a throwaway directory first (audit
    finding: the test used fixed Telegram-like uids inside the real data
    directory and rmtree'd them)."""
    global send, think, DATA
    import tempfile
    DATA = Path(tempfile.mkdtemp(prefix='quit-sponsor-selftest-'))
    sent = []
    send = lambda chat, text, markup=None: sent.append(text)
    think = lambda uid, text, hist=None: '[sponsor reply to: %s]' % text
